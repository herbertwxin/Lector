...
ParsedUri parse_uri(fz_context* mupdf_context, std::string uri) {
    // Basic implementation of pdf_parse_link_uri
    fz_link_dest val = fz_make_link_dest_none();
    const char* str = uri.c_str();
    const char* page = strstr(str, "page=");
    const char* zoom = strstr(str, "zoom=");
    const char* rect = strstr(str, "viewrect=");

    if (page) {
        val.loc.page = atoi(page + 5) - 1;
        if (val.loc.page < 0) val.loc.page = 0;
    }

    if (zoom) {
        const char* zoom_data = zoom + 5;
        char* end;
        val.zoom = strtof(zoom_data, &end);
        if (*end == ',') {
            val.x = strtof(end + 1, &end);
            if (*end == ',') {
                val.y = strtof(end + 1, &end);
            }
        }
    } else if (rect) {
        const char* rect_data = rect + 9;
        char* end;
        val.x = strtof(rect_data, &end);
        if (*end == ',') {
            val.y = strtof(end + 1, &end);
        }
    }

	return { val.loc.page + 1, val.x, val.y };
}
...
void split_key_string(std::wstring haystack, const std::wstring& needle, std::vector<std::wstring> &res) {
	...
	while ((loc = haystack.find(needle)) != (size_t) -1) {
		...
		if ((loc < (haystack.size()-1)) &&  (haystack.substr(loc + needle_size, needle.size()) == needle)) {
...
std::wstring truncate_string(const std::wstring& inp, int size) {
		...
		std::wstring truncated = inp.substr(0, size - 3);
	while (!truncated.empty() && truncated.back() == L' ') {
		truncated.pop_back();
	}
	return truncated + L"...";
	}
