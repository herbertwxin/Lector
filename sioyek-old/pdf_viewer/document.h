...
private:
	...
	DatabaseManager* db_manager = nullptr;
	CachedChecksummer* checksummer;
	std::unordered_map<std::wstring, std::weak_ptr<Document>> cached_documents;
...
public:
	...
	std::shared_ptr<Document> get_document(const std::wstring& path);
	std::shared_ptr<Document> find_document(const std::wstring& path);
	void free_document(std::shared_ptr<Document> document);
	//const std::unordered_map<std::wstring, Document*>& get_cached_documents();
	std::vector<std::shared_ptr<Document>> get_active_documents();
