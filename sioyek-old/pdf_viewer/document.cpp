...
std::shared_ptr<Document> DocumentManager::get_document(const std::wstring& path) {
	if (cached_documents.find(path) != cached_documents.end()) {
		std::weak_ptr<Document> weak_doc = cached_documents.at(path);
		std::shared_ptr<Document> doc = weak_doc.lock();
		if (doc) {
			return doc;
		}
	}
	std::shared_ptr<Document> new_doc(new Document(mupdf_context, path, db_manager, checksummer));
	cached_documents[path] = new_doc;
	return new_doc;
}

std::shared_ptr<Document> DocumentManager::find_document(const std::wstring& path) {
	if (cached_documents.find(path) != cached_documents.end()) {
		std::weak_ptr<Document> weak_doc = cached_documents.at(path);
		return weak_doc.lock();
	}
	return nullptr;
}

std::vector<std::shared_ptr<Document>> DocumentManager::get_active_documents() {
	std::vector<std::shared_ptr<Document>> res;
	for (auto it = cached_documents.begin(); it != cached_documents.end(); ) {
		std::shared_ptr<Document> doc = it->second.lock();
		if (doc) {
			res.push_back(doc);
			++it;
		}
		else {
			it = cached_documents.erase(it);
		}
	}
	return res;
}
...
void DocumentManager::delete_global_mark(char symbol) {
	std::vector<std::shared_ptr<Document>> docs = get_active_documents();
	for (auto doc : docs) {
...
void DocumentManager::free_document(std::shared_ptr<Document> document) {
	if (!document) return;
	std::wstring path_to_erase = document->get_path();
