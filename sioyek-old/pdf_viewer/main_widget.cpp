...
void MainWidget::update_link_with_opened_book_state(Portal lnk, const OpenedBookState& new_state) {
    ...
    std::shared_ptr<Document> link_owner = document_manager->get_document(docpath);
    ...
    if (current_history_index >= 0) {
        ...
        */
        if (link_to_edit) {
            ...
            std::shared_ptr<Document> link_owner = document_manager->get_document(link_document_path);
...
void MainWidget::toggle_window_configuration() {
    ...
    if (helper_opengl_widget == nullptr) return;
...
QRect MainWidget::get_helper_window_rect() {
    if (helper_opengl_widget == nullptr) return QRect();
...
void MainWidget::on_new_paper_added(const std::wstring& file_path) {
    if (is_pending_link_source_filled()) {
        ...
        std::shared_ptr<Document> new_doc = document_manager->get_document(file_path);
...
void MainWidget::changeEvent(QEvent* event) {
    ...
    if (event->type() == QEvent::ApplicationStateChange) {
        ...
        if (state == Qt::ApplicationSuspended || state == Qt::ApplicationHidden) {
            if (validation_interval_timer && validation_interval_timer->isActive()) {
...
void MainWidget::show_textbar(const std::wstring& name, bool fill_selected) {
    ui_manager->show_textbar(name, fill_selected);
}

void MainWidget::hide_textbar() {
    ui_manager->hide_textbar();
}
...
void MainWidget::add_portal(std::wstring source_path, Portal new_link) {
	if (source_path == main_document_view->get_document()->get_path()) {
		...
		std::shared_ptr<Document> doc = document_manager->find_document(source_path);
		if (doc) {
    ...
    if (text[0] == '#') {
        ...
        if (parts.size() == 2) {
            ...
            if ((begin_parts.size() == 3) && (end_parts.size() == 3)) {
				...
				input_processor->handle_left_click(begin_window_pos, true, false, false, false);
				input_processor->handle_left_click(end_window_pos, false, false, false, false);
            }
        }
        ...
        if (parts.size() == 1) {
            ...
            if (schar_rects.size() > 1){
				...
				input_processor->handle_left_click({ (srect.x0 + srect.x1) / 2 - 1, (srect.y0 + srect.y1) / 2 }, true, false, false, false);
				input_processor->handle_left_click({ erect.x0 , (erect.y0 + erect.y1) / 2 }, false, false, false, false);
				opengl_widget->set_should_highlight_words(false);
            }
        }
        if (parts.size() == 2) {
            ...
            if ((schar_rects.size() > 0) && (echar_rects.size() > 0)) {
				...
				input_processor->handle_left_click({ (srect.x0 + srect.x1) / 2 - 1, (srect.y0 + srect.y1) / 2 }, true, false, false, false);
				input_processor->handle_left_click({ erect.x0 - w/2 , (erect.y0 + erect.y1) / 2 }, false, false, false, false);
				opengl_widget->set_should_highlight_words(false);
            }
				...
				input_processor->handle_left_click({ srect.x0 + 5, (srect.y0 + srect.y1) / 2 }, true, false, false, false);
				input_processor->handle_left_click({ erect.x0 - 5 , (erect.y0 + erect.y1) / 2 }, false, false, false, false);
				opengl_widget->set_should_highlight_words(false);
            }
	...
	if (new_command) {
		if (!new_command->next_requirement(this).has_value()) {
			...
			if (next_requirement.type == RequirementType::Text) {
				ui_manager->show_textbar(utf8_decode(next_requirement.name), true);
			}
...
void MainWidget::handle_overview_to_portal() {
	if (opengl_widget->get_overview_page()) {
		...
		if (portal_) {
			...
			if (destination_path) {
				std::shared_ptr<Document> doc = document_manager->get_document(destination_path.value());
				if (doc) {
					overview_state.absolute_offset_y = portal.dst.book_state.offset_y;
					overview_state.doc = doc.get();
