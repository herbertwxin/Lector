...
extern std::wstring DICTIONARY_URL;
...
class ChapterSearchCommand : public TextCommand {
	...
	void pre_perform(MainWidget* widget) {
		...
		if (chapter_range) {
			...
			widget->ui_manager->text_command_line_edit->setText(search_range_string.str().c_str() + widget->ui_manager->text_command_line_edit->text());
		}
...
class DictionaryLookupCommand : public Command {
	void perform(MainWidget* widget) {
		if (widget->selected_text.size() > 0 && DICTIONARY_URL.size() > 0) {
			search_custom_engine(widget->selected_text, DICTIONARY_URL);
		}
	}
	std::string get_name() {
...
class ToggleScrollbarCommand : public Command {
	...
	void perform(MainWidget* widget) {
        widget->ui_manager->toggle_scrollbar();
	}
...
class ToggleStatusbarCommand : public Command {
	...
	void perform(MainWidget* widget) {
		widget->ui_manager->toggle_statusbar();
	}
...
CommandManager::CommandManager(ConfigManager* config_manager) {
	...
	new_commands["dictionary_lookup"] = []() {return std::make_unique< DictionaryLookupCommand>(); };
