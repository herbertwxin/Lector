...
std::wstring DICTIONARY_URL = L"https://www.google.com/search?q=define+";
...
int main(int argc, char* args[]) {
	...
	// Process any pending QFileOpenEvent from macOS before handling default args.
	// On macOS, opening a file via Finder sends a QFileOpenEvent instead of using
	// command-line arguments. Without this, handle_args would open the last-opened
	// document first, and the QFileOpenEvent arriving later would open the actual
	// file in a second window when should_launch_new_window is enabled.
	app.processEvents();

	bool deferred_startup = false;
	if (windows[0]->doc() == nullptr) {
		QStringList initial_args = app.arguments();
		if (!app.pending_file_name.isEmpty()) {
			initial_args << app.pending_file_name;
			app.pending_file_name.clear();
		}

#ifdef Q_OS_MACOS
		// Check if there are explicit file arguments on the command line.
		QCommandLineParser* startup_parser = get_command_line_parser();
		startup_parser->parse(initial_args);
		bool has_file_arg = startup_parser->positionalArguments().size() > 0;
		delete startup_parser;

		if (has_file_arg) {
			handle_args(initial_args);
		}
		else {
			// On macOS, when no file is specified on the command line, the file
			// may arrive later via QFileOpenEvent (e.g. when opened from Finder).
			// Defer loading the default (last-opened) file to give the event time
			// to arrive. If the QFileOpenEvent arrives during this delay, the
			// file_ready handler will open the file in windows[0], and this
			// deferred callback will see doc() != nullptr and do nothing.
			deferred_startup = true;
			QTimer::singleShot(500, [&app, main_widget]() {
				if (windows[0]->doc() == nullptr) {
					QStringList deferred_args = app.arguments();
					if (!app.pending_file_name.isEmpty()) {
						deferred_args << app.pending_file_name;
						app.pending_file_name.clear();
					}
					handle_args(deferred_args);
				}
