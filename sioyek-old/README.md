...
### Mac (Native Swift — Apple Silicon)
A native macOS version built entirely in Swift using SwiftUI and PDFKit. This version has zero third-party dependencies (no Qt, MuPDF, or OpenGL) and runs natively on Apple Silicon.
...
git clone --recursive https://github.com/ahrm/sioyek
cd sioyek
chmod +x build_mac_native.sh
./build_mac_native.sh
```

Or build directly with Swift Package Manager:
```zsh
cd macos-native
swift build -c release --arch arm64
```

**Key differences from the Qt version:**
- Uses native PDFKit for PDF rendering (no MuPDF)
- Uses SwiftUI for the user interface (no Qt)
- Uses native macOS dark mode support
- Native Retina display support
- Reads the same `prefs.config` / `keys.config` configuration files
- Bookmarks and highlights stored as JSON in `~/Library/Application Support/sioyek/`

**Supported features:**
- PDF viewing with continuous scrolling
- Vim-like keyboard navigation (`j`/`k`, `gg`/`G`, `Space`, `/` search)
- Table of contents sidebar
- Bookmarks (add/delete/navigate)
- Highlights with multiple types
- Dark mode toggle
